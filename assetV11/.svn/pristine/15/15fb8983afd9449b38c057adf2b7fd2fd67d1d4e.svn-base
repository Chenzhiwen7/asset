define([
	...
	function getCharge(begin_time, end_time, vmodel) {
		websocket.send({
			...
			vmodel.total_amount = Index.getMoney(data.total_actual_receivable);
	...
	var body = avalon.define({
		...
		$personalWinOpts: {
			...
			afterShow: function(isInit, vmodel) {
				if(isInit) {
					...
					$("#personal-eDatePicker2").datetimepicker(Index.getDateTimePickerOpts({
							startView: 2
						}))
						.on("changeDate", function() {
							setTimeout(function() {
		...
		$registerOpts: {
			...
			buttons: [{
				...
				theme: "primary",
				handler: function(vmodel) {
					var a = vmodel.register_code;
					// avalon.log(vmodel.register_code);
					websocket.send({
						command: "REGISTER_INFOR",
						biz_content: {
							register_id: Index.register_id_union,
							register_code: a,
						}
					}, document.body, function(data) {
						if(data.code === '0' && data.msg === "ok") {
							Index.alert('注册成功');
							top.registerstate = "已注册";
			...
			// }],
			afterShow: function(isInit, vmodel) {
				//alert(Index.register_id_union);
				vmodel.register_id = Index.register_id_union;
				vmodel.register_code = "";
				//vmodel.register_id = data.register_id;
			},
			register_code: "",
			register_id: "",
		},
			...
			buttons: [{
				...
				handler: function(vmodel) {
					...
					websocket.send({
						...
						// 	total_parking_space_remaining : vmodel.total_parking_space_remaining,
	...
	var top = avalon.define({
		...
		UserName1: window.localStorage.UserName1,
		total_parking_space_remaining: "--",
		total_parking_space: "--",
		correctNum: function() {
	...
	function dealParkingSpaceData(data) {
		...
		top.parking_lot_name = data.parking_lot_name;
	}
	//实时更新停车位信息
	...
	// //--add下发注册信息
	// function register(id,code,func)
	// {
	...
	// }
	// //add 实时监控注册状态
	websocket.callbacks.PUSH_REGISTER_INFOR = function(data) {
		//RegisterData.data=data;
		Index.register_id_union = data.register_id;
		//alert(data.register_id);
		if(data.register_bool === '0') {

			if(data.datalinedays === '0') {
				//alert("请注册");
				if(top.registerstate !== "禁用")
					alert("系统试用期已过，请及时注册");
				top.registerstate = "禁用";
			} else {
				top.registerstate = "试用剩" + data.datalinedays + "天";
			}
		} else {
			top.registerstate = "已注册";
			//alert(top.registerstate);
		}

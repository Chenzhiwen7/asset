...
require([
	...
	var content = avalon.define({
		...
		search: function() {
			...
			Index.websocket.send({
				command: "GET_One_Customer_Info",
				biz_content: {
					one_page_count	: 10,
					page_from		: 1,
					customer		:content.searchName,
					licence			:content.searchCarNum
				}
			}, document.body, function(data) {				
				if(data.code === "0" && data.msg === "ok") {
					var V_C_L=data.vip_car_list					
					for(var i=0,ii;ii=V_C_L[i++];){
						for(var j=0,jj;jj=getUserCarData.permission_group_list[j++];){
					...
					avalon.vmodels.$userTb.loadFrontPageDataEx(V_C_L);
				}
			})
		},		
		searchName : "",
		...
		$userTbOpts : {
			...
			columns : [
				...
				// {title : "修改人",field : "add_by"},
				// {title : "修改时间",field : "remark"}
			]
		},
		parkingName : '--',
		/******************************************************************/
		...
		$userManageWinOpts : {
			...
			search : function(){
				...
				Index.websocket.send({
					command: "GET_One_Customer_Info",
					biz_content: {
						customer 		: vmodel.customer_name,
						licence 		: vmodel.customer_telphone			
					}
				}, document.body, function(data) {
					var list = data.vip_car_list;
					avalon.vmodels.$user.loadFrontPageDataEx(list);	
				});
			},
			...
			afterShow : function(isInit,vmodel){
				...
				//切换页面:用户列表
				avalon.vmodels.$user.toPageExFunc = function loadDataByPage(page, pagesize) {
					Index.websocket.send({
						...
						avalon.vmodels.$user.LoadIndexPageData(list, page);					
					});
				};
				avalon.vmodels.$user.$toPage = avalon.vmodels.$user.$toPageEx;
				avalon.vmodels.$user.$changePageSize = avalon.vmodels.$user.$changePageSizeEx;
				avalon.vmodels.$user.$toThePage = avalon.vmodels.$user.$toThePageEx;
			}
		},
		$userWinOpts : {
			title : '',
		...
		$carWinOpts : {
			...
			model : {
				...
				add_by : Index.top.accountName,
				remark : avalon.filters.date(new Date(),"yyyy-MM-dd HH:mm:ss"),
			},
	...
	//设置vip_customer_list，vip_car_list，permission_group_list，permission_group_mapping_list
	//和返回vip_car_list
	function getUserCarData(data){
		avalon.scan();
		VIP_CAR_NUMBER=data.vip_car_number;
		var vip_customer_list = data.vip_customer_list;		
		var vip_car_list = data.vip_car_list;
		var permission_group_list = data.permission_group_list;
		getUserCarData.permission_group_list = data.permission_group_list;
		var permission_group_mapping_list = data.permission_group_mapping_list;
		var maxSeq = '0';
		for(var i=0,ii;ii=vip_car_list[i++];){
			if(+ii.vip_car_seq > +maxSeq){
			...
			for(var j=0,jj;jj=permission_group_list[j++];){
				if(jj.channel_permission_group_seq === ii.channel_permission_group_seq){
					ii.channel_permission_group_name = jj.channel_permission_group_name;
					break;
				}
	...
	function setUserTbData(data) { 
		VIP_CAR_NUMBER=data.vip_car_number;
		avalon.vmodels.$userTb.initPageData(VIP_CAR_NUMBER);	
		avalon.vmodels.$userTb.loadFrontPageDataEx(getUserCarData(data));	
	}
	// 刷新用户管理面的拥有车辆管理列表
	...
	function setUserData(){
		...
		Index.websocket.send({
			...
			setUserData.data = data.vip_car_list;
			var list = data.vip_customer_list;
			var number = data.customer_number;
			var maxSeq = '0';
				for(var i=0,ii;ii=list[i++];){
					if(+ii.vip_customer_seq > +maxSeq){
						maxSeq = ii.vip_customer_seq;
			...
			setUserData.maxSeq = maxSeq;
			avalon.vmodels.$user.initPageData(number);
			avalon.vmodels.$user.loadFrontPageDataEx(list);
		});
	}
	...
	//切换页面:VIP用户列表
	avalon.vmodels.$userTb.toPageExFunc = function loadDataByPage(page, pagesize) {
		Index.websocket.send({
			command: "GET_ALL_Customer_Info",
			biz_content: {
				page_from: page,
			...
			if(data.code === "0" && data.msg === "ok") {
				...
				avalon.vmodels.$userTb.LoadIndexPageData(vip_car_list, page);
			}
		});
	};
	avalon.vmodels.$userTb.$toPage = avalon.vmodels.$userTb.$toPageEx;
	avalon.vmodels.$userTb.$changePageSize = avalon.vmodels.$userTb.$changePageSizeEx;
	avalon.vmodels.$userTb.$toThePage = avalon.vmodels.$userTb.$toThePageEx;

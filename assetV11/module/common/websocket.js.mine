define(function(){
	...
	// var host = "192.168.2.7";
	   var host = "192.168.134.231";
	   var hostHttp = "192.168.2.78"
	 // var host = "192.168.2.222";
//		var host = "192.168.2.140";   
//	 var host = "127.0.0.1";
	//var host = "dwdtec.vicp.net";  
//	 var host=window.location.host;
	var urlHttp="http://" + hostHttp + ":8089/pay/payByCard";
	var urlHttpAgain="http://" + hostHttp + ":8089/pay/payByCards";
	
//	var urlHttp="http://" + host + ":8089/pay/payByCard";
//	var urlHttpAgain="http://" + host + ":8089/pay/payByCards";
	
	var url = "ws://" + host + ":51601";
	var socket;
	//出错后重新连接时间
	var errorReconectTime = 1000;
	var socketHelper = {
		post : function(urlHttps,jsonData,func){
			$.post(urlHttps,jsonData,func)
		},
		getUrlHttp:function(){
			return urlHttp;
		},
		getUrlHttpAgain:function(){
			return urlHttpAgain;
		},
		//拍照文件夹
	...
	OrderHelper.prototype.send = function(){
		...
		if(!socket){
			...
			socket.onerror = function(e){
				...
				socket.close();
				socket = null;
				window.setInterval(reconnect_watch,errorReconectTime);
				//window.location.reload();
			};
